...
#include <iomanip>
using namespace std;
const string ClientsFileName = "Bank1.txt";
void ShowMainMenue();
...
bool DepAnswer(char ans , double Dep , vector <sClient> & vClient , string AccountNumber){

    if(toupper(ans) == 'Y'){
        
        for(sClient & C : vClient){
            if(C.AccountNumber == AccountNumber){
                C.AccountBalance = C.AccountBalance + Dep ;
...
void deposit( vector <sClient> & vClient){
    ...
    do{
        string AccountNumber = ReadClientAccountNumber();
         Y = FindClientByAccountNumber(AccountNumber,vClient,Client);

         if(Y == true){
            PrintClientCard(Client);
            Dep = ReadDeposit();
            char ans = ReadAnswer();
            DepAnswer(ans , Dep , vClient , AccountNumber);
...
void WithDraw(vector <sClient> & vClient){
    ...
    do{
        ...
        if(Y == true){
            PrintClientCard(Client);
            withdraw = Readwihtdraw();
            char ans = ReadAnswer();
            WithDrawAnswer(ans,withdraw,vClient,AccountNumber);
...
void PerFormMainTransactions(enMainTransactions MainTransactions)
{
...
void TransactiosMenueScreen(){
    ...
    cout << "\n\t [4] Main Menue.";
    cout << "\n=======================================\n";
    PerFormMainTransactions((enMainTransactions) ReadMainTransactions());
}
